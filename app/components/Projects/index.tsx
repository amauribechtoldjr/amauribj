...
function ProjectsCard({ img, project, skills, githubLink }: ProjectsCardProps) {
  return (
    <div className="my-8">
      <div className="">
        {project.url ? (
          <a href={project.url} className="" target="_blank" rel="noreferrer">
            <h2 className="font-bold my-4 text-1xl underline">
              {project.name}
            </h2>
          </a>
        ) : (
          <h2 className="font-bold my-4 text-1xl">{project.name}</h2>
        )}
